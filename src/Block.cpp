...
*/
void Block::update(std::vector<Block *> block)
{
	if (this->getActive())
	{
	...
	else if (!this->getActive() && !this->getHasDroppedPowerUp())
	{
		...
		this->powerup->getActive() &&
		this->getHasDroppedPowerUp())
	{
		powerup->update();
	}
}
...
bool Block::getHasDroppedPowerUp()
{
	return this->hasDroppedPowerUp;
}

void Block::setHasDroppedPowerUp(bool hasDroppedPowerUp)
{
	this->hasDroppedPowerUp = hasDroppedPowerUp;
}

int Block::getDropChance()
{
	return this->dropChance;
}

void Block::setDropChance(int dropChance)
{
...
void Block::setHitPoints( int hitPoints )
{
...
void Block::setTextureID( int textureID )
{
