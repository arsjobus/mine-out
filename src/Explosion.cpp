...
{
	return this->getGlobalBounds().intersects(block.getGlobalBounds());
}
...
bool Explosion::nextFrame()
{
	...
	if (dt.asMilliseconds() >= sf::milliseconds(20).asMilliseconds())
	{
		if (++currentFrame.x < totalFrames.x)
			this->setTextureRect(sf::IntRect(currentFrame.x * frameSize.x, currentFrame.y * frameSize.y, 256, 256));
		else
		{
			currentFrame.x = 0;
...
void Explosion::loadDefaultSettings()
{
	...
	sndExplosion.setBuffer(resource.getBufferedSound( 1 ));
	...
	this->setTextureRect(sf::IntRect(this->currentFrame.x,this->currentFrame.y,256,256));
