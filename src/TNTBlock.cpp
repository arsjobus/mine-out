...
void TNTBlock::update(std::vector<Block *> block)
{
	...
	if ( !this->getActive() && !this->getHasExploded() )
	{
			...
			dt.asMilliseconds() >= sf::milliseconds(250).asMilliseconds())
		{
			...
			{
				if (block[i]->getActive() &&
					explosion->detectCollisionWithBlocks(*block[i]))
				{
...
float TNTBlock::getExplosiveRadius()
{
	return this->explosiveRadius;
}

void TNTBlock::setExplosiveRadius(float explosiveRadius)
{
	this->explosiveRadius = explosiveRadius;
}

bool TNTBlock::getHasExploded()
{
	return this->hasExploded;
}

void TNTBlock::setHasExploded(bool hasExploded)
{
	this->hasExploded = hasExploded;
}

Explosion *TNTBlock::getExplosion()
{
